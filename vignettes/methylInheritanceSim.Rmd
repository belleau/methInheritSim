...
The length of the list corresponds to the number of generation. The generation 
are stored in order (first entry = first generation, second entry = second 
generation, etc..). All samples related to one generations are contained in a 
...
* The second file contains a **S4 calculateDiffMeth** object generate with 
...
DMS datasets. Several simulator parameters can be derived from real dataset 
provided by the user in order to replicate realistic case-control scenarios. 
